	...
	this.addUser = function(newUser)
						{
							...
							// Tell everyone in room that a new user has arrived
							this.broadcast('newUser', newUser.id);

							// Run order if it isn't already running
							if(!this.orderIsRunning)
...
function auth(socket, info)
{
	...
	else
	{
		...
		// Debug message
		console.log('Created ' + user.getName() + ' at ' + user.getSocket() + ' in ' + user.getRoom().getName());
		// Add to room
		room.addUser(new User(info.uname, socket, room));
		// Woohoo start
		socket.emit("youin");
	}
}
